...
On-premise infrastructure: Advantage:
...
IAM: Identity and access management: IAM has two parts: Authentication and authorization.
...
1) Global services: IAM, S3 (even though data is stored is region specific but buckets are unique, so it can be accessed from any region)
Route 53, Cloudfront, AWS organization, WAF (web application firewall), Billing and cost managment, support center.

2) Regional services: These service are specific to regions.
EC2, VPC, RDS etc.


AWS organization: It is a service of AWS that helps you centrally manage multiple AWS accounts. It is mainly used by companies to organize, control and govern their cloud resources at scale.
...
Service quotas: Here it has list of all the quotas for all the AWS resources. That means, it lists limitation of the services.
...
AWS Account: Here you’ll manage your account. It has option to change your full name, company name, address, phone number, website URL.
